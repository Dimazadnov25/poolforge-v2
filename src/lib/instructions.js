...
export function buildIncreaseLiquidityIx(
  ...
  tickArrayLower, tickArrayUpper, liquidityAmount, tokenMaxA, tokenMaxB, tickArrayCurrent = null
) {
  ...
  const hasCurrent = tickArrayCurrent &&
  ...
  const remainingInfo = hasCurrent
    ? Buffer.from([0x01, 0x01, 0x00, 0x00, 0x00, 0x06, 0x01])
    : Buffer.from([0x00]);
  ...
  if (hasCurrent) {
    keys.push({ pubkey: tickArrayCurrent, isSigner: false, isWritable: true });
  }

  return new TransactionInstruction({ programId: WHIRLPOOL_PROGRAM, keys, data });
}
