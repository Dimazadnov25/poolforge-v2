...
export function usePool() {
  ...
  const openPosition = useCallback(async ({ priceLower, priceUpper, solAmount }) => {
    ...
    try {
      ...
      await connection.confirmTransaction({ signature: sig1, blockhash: bh1, lastValidBlockHeight: lv1 }, 'finalized')
      ...
      tx2.add(buildIncreaseLiquidityIx(wallet.publicKey, positionPDA, positionTokenAccount, SOL_USDC_WHIRLPOOL, tokenOwnerA, tokenOwnerB, VAULT_A, VAULT_B, tickArrayLower, tickArrayUpper, liquidityAmount, Math.floor(lamports * 1.1), Math.floor(usdcRaw * 10)))
