...
export default function OpenPositionForm({ pool, solPrice, onOpen, loading }) {
  ...
  const handleMaxSol = (e) => {
    e.preventDefault()
    e.stopPropagation()
    const max = Math.max(0, (pool?.solBalance || 0) - 0.03)
  ...
  const handleMaxUsdc = (e) => {
    ...
    const sqrtPl = Math.sqrt(pl * 1e-3)
    const sqrtPu = Math.sqrt(pu * 1e-3)
    const solAvail = Math.max(0, (pool?.solBalance || 0) - 0.03)
    ...
    <div className="card" style={{marginTop:'1rem'}}>
      ...
      {usdc !== null && solAmount && (
        <div style={{
          ...
          USDC ben�tigt: {usdc.toFixed(2)} � Verf�gbar: {usdcAvail.toFixed(2)} {hasEnough ? '?' : '?'}
        </div>
      )}
