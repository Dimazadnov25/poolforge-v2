...
export default function PositionDetails({ position, poolState, solBalance, usdcBalance, solPrice, fetchPosition, onClose, onCollect, onAddLiquidity, onRebalance, onUpdate, onUpdateFees }) {
  ...
  return (
    <div className="card" style={{marginTop:'0.5rem'}}>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0.5rem', marginBottom:'0.75rem', width:'100%'}}>
      ...
      <div style={{marginBottom:'0.5rem'}}>
        <div style={{display:'flex', gap:'0.5rem', alignItems:'center', marginBottom:'0.25rem'}}>
          ...
          <div style={{display:'flex',gap:'0.25rem'}}>
            <button type="button" onClick={(e)=>{e.stopPropagation();setAddAmount(Math.max(0,(solBalance||0)-0.03).toFixed(4))}} style={{padding:'0.2rem 0.4rem',borderRadius:'6px',border:'1px solid var(--border)',background:'var(--surface)',color:'var(--text)',cursor:'pointer',fontSize:'0.7rem'}}>MAX SOL</button>
            <button type="button" onClick={(e)=>{e.stopPropagation();if(!details||!poolState?.currentPrice) return;const p=poolState.currentPrice;const pl=details.priceLower;const pu=details.priceUpper;const sqrtP=Math.sqrt(p*1e-3);const sqrtPl=Math.sqrt(pl*1e-3);const sqrtPu=Math.sqrt(pu*1e-3);const usdcAvail=usdcBalance||0;const liq=usdcAvail*1e6/(sqrtP-sqrtPl);const solNeeded=liq*(1/sqrtP-1/sqrtPu)/1e9;setAddAmount(Math.min(solNeeded,Math.max(0,(solBalance||0)-0.03)).toFixed(4))}} style={{padding:'0.2rem 0.4rem',borderRadius:'6px',border:'1px solid var(--border)',background:'var(--surface)',color:'var(--text)',cursor:'pointer',fontSize:'0.7rem'}}>MAX USDC</button>
          </div>
        </div>
