...
export default function PositionDetails({ position, poolState, solBalance, fetchPosition, onClose, onCollect, onAddLiquidity, onRebalance, onUpdate, onUpdateFees }) {
  ...
  return (
    <div className="position-card">
      ...
      <div>
        <div style={{display:'flex',gap:'0.5rem',marginBottom:'0.5rem'}}><button onClick={()=>setAddAmount(Math.max(0,(solBalance||0)-0.01).toFixed(4))} style={{padding:"0.2rem 0.4rem",borderRadius:"6px",border:"1px solid var(--border)",background:"var(--surface)",color:"var(--text)",cursor:"pointer",fontSize:"0.7rem",marginBottom:"0.25rem"}}>MAX</button>
