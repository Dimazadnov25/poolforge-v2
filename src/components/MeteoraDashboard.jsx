...
export default function MeteoraDashboard({solPrice}){
  ...
  if(!data)return<div style={{background:"var(--card)",borderRadius:"12px",padding:"1.25rem",marginBottom:"1rem",color:"var(--muted)",fontSize:"0.85rem"}}>ðŸŒŠ Meteora wird geladen...</div>
    ...
    <div style={{background:"var(--card)",borderRadius:"12px",padding:"1.25rem",marginBottom:"1rem"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"1rem"}}>
        ...
        <span style={{padding:"0.2rem 0.7rem",borderRadius:"20px",fontSize:"0.8rem",fontWeight:"bold",background:data.inRange?"rgba(0,200,100,0.15)":"rgba(239,68,68,0.15)",color:data.inRange?"#00c864":"#ef4444"}}>
          {data.inRange?"âœ… In Range":"ðŸš¨ Out of Range"}
        </span>
      </div>
      ...
      <div style={{marginBottom:"1rem"}}>
        <div style={{display:"flex",justifyContent:"space-between",fontSize:"0.72rem",color:"var(--muted)",marginBottom:"0.3rem"}}>
          ...
          <span style={{color:"#00d4ff",fontWeight:"bold"}}>â— {data.activeBin}</span>
        ...
        <div style={{display:"flex",justifyContent:"space-between",fontSize:"0.7rem",marginTop:"0.3rem"}}>
          <span style={{color:dangerLower?"#ef4444":"var(--muted)"}}>â¬… {data.binsToLower} Bins ({pctLower}%)</span>
          <span style={{color:dangerUpper?"#ef4444":"var(--muted)"}}>{ data.binsToUpper} Bins ({pctUpper}%) âž¡</span>
        </div>
      </div>      <a href="https://app.meteora.ag/dlmm/5rCf1DM8LjKTw4YqhnoLcngyZYeNnQqztScTogYHAS6?position=2pLfC12zAeZD1CCE7q4ksxwj84h4kuZP2W5dM1u8Cgtt" target="_blank" rel="noopener noreferrer" style={{display:"block",textAlign:"center",padding:"0.6rem",borderRadius:"8px",background:"rgba(0,200,100,0.15)",color:"#00c864",fontWeight:"bold",fontSize:"0.85rem",textDecoration:"none",marginBottom:"0.75rem",border:"1px solid rgba(0,200,100,0.3)"}}>🔄 Rebalancen auf Meteora ↗</a>
      {/* Stats */}
