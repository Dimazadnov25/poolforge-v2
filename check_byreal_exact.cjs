...
async function check() {
  ...
  for (const { account } of accounts.value) {
    ...
    // Jeden einzelnen byte-offset prüfen für SOL fee (~4860000)
    console.log('SOL fee Kandidaten (alle offsets):')
    for (let i = 0; i <= pd.length - 8; i++) {
      const v = Number(pd.readBigUInt64LE(i))
      if (v >= 4000000 && v <= 7000000) {
        console.log(`  offset ${i}: ${v} = ${(v/1e9).toFixed(6)} SOL`)
      }
    }
  }
}
